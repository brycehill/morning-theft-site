...
// TODO USE STREAMS AND ADD CACHING AND INFINITE SCROLL/PAGIN
...
exports.getAll = function(cb) {
    ...
    async.map(methods, get, function (err, results) {
        var users = results[0].concat(results[1]),
            ...
            photos = relevantTags.concat(recent);

        // Sort's ascending, but we want newest first.
        photos.sort(function(a, b) {
            return a.ts - b.ts;
        });

        cb(null, photos.reverse());
    });
};
